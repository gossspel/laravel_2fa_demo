...
class GoogleAuthenticator {
    ...
    private static function _generateKey($key, $counter, $digits = 6) {
        ...
        return GoogleAuthenticator::_keyToString($hash, $digits);
    }
    ...
    public static function generate($key, $window, $timestamp = false) {
        if (!$timestamp && $timestamp !== 0) {
            $timestamp = GoogleAuthenticator::_getTime();
        }

        $counter = intval($timestamp / $window);
        
        return GoogleAuthenticator::_generateKey(GoogleAuthenticator::_base32_decode($key), $counter);
    }
    ...
    public static function getQRcodeURL($user, $host, $secret) {
        ...
        $url = "https://chart.googleapis.com/chart?cht=qr&chs=200x200&chl=%s&choe=UTF-8&chld=M|0";
